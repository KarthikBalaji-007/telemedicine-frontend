  ...
  const downloadJSONReport = (dateStr, timeStr) => {
    const reportData = {
      ...
      diagnosis: {
        ...
        confidenceLevel: diagnosis?.probability || 'Unknown',
        riskLevel: diagnosis?.riskLevel || 'Unknown',
        description: diagnosis?.description || 'No analysis available',
        causes: diagnosis?.causes || 'No causes available',
...
<head>
    ...
    <style>
        ...
        .risk-${diagnosis?.riskLevel?.toLowerCase() || 'unknown'} { 
            color: white; 
            padding: 5px 15px; 
            border-radius: 20px; 
            font-weight: bold;
...
<body>
    ...
    <div class="section">
        ...
        <p><strong>Likely Condition:</strong> ${diagnosis?.condition || 'Unknown'}</p>
        <p><strong>Confidence Level:</strong> ${diagnosis?.probability || 'Unknown'}</p>
        <p><strong>Risk Level:</strong> <span class="risk-${diagnosis?.riskLevel?.toLowerCase() || 'unknown'}">${diagnosis?.riskLevel || 'Unknown'}</span></p>
    </div>

    <div class="section">
        <h3>🔍 Analysis</h3>
        <p>${diagnosis?.description || 'No analysis available'}</p>
    </div>

    <div class="section">
        <h3>📖 Causes</h3>
        <p>${diagnosis?.causes || 'No causes available'}</p>
    </div>

    <div class="section">
        <h3>📝 Recommended Next Steps</h3>
        <ol class="steps">
            ${diagnosis?.nextSteps?.map(step => `<li>${step}</li>`).join('') || '<li>No recommendations available</li>'}
        </ol>
    </div>
  ...
  const downloadPDFReport = (dateStr, timeStr) => {
    // Create a new window with the report content for print-to-PDF
    ...
    printWindow.document.write(`
      ...
      <body>
        ...
        <div class="section">
          ...
          <p><strong>Likely Condition:</strong> ${diagnosis?.condition || 'Unknown'}</p>
          <p><strong>Confidence Level:</strong> ${diagnosis?.probability || 'Unknown'}</p>
          <p><strong>Risk Level:</strong> ${diagnosis?.riskLevel || 'Unknown'}</p>
        </div>
        ...
        <div class="footer">
          <p><strong>Created by MediChat - AI Medical Assistant</strong></p>
          <p><strong>DISCLAIMER:</strong> This report is generated by MediChat AI and is for informational purposes only.</p>
          <p>© ${new Date().getFullYear()} MediChat. All rights reserved.</p>
        </div>
