...
import { Stethoscope, Send, Upload, Camera, FileText } from 'lucide-react';
...
const HomePage = () => {
  ...
  const handleFileUpload = (event) => {
    ...
    if (file) {
      ...
      navigate('/chat', { 
        state: { 
          initialSymptom: `I have uploaded a ${file.type.includes('image') ? 'medical image' : 'medical report'}: ${file.name}`,
          uploadedFile: file
        } 
      });
    }
        ...
        <div className="w-full max-w-4xl mx-auto mb-8">
          <div className="bg-gray-50 border-2 border-dashed border-gray-300 rounded-2xl p-8 text-center">
            <div className="flex justify-center items-center mb-4">
              <Camera className="w-12 h-12 text-medical-red mr-4" />
              <FileText className="w-12 h-12 text-blue-600" />
            </div>
            <h3 className="text-xl font-bold text-gray-700 mb-2">Upload Medical Files</h3>
            <p className="text-gray-500 mb-6">Upload photos of symptoms, test results, or medical reports for AI analysis</p>
            
            <label className="inline-flex items-center bg-medical-red text-white px-6 py-3 rounded-full font-medium hover:bg-red-600 transition-colors cursor-pointer">
            ...
            <div className="mt-4 text-sm text-gray-500">
              Supported: Images (JPG, PNG), PDF reports, Medical documents
            </div>
          </div>
        </div>
