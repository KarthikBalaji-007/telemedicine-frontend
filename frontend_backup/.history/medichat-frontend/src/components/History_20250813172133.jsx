        ...
        <div className="w-full max-w-6xl mx-auto space-y-4">
          {sortedHistory.length === 0 ? (
            ...
            sortedHistory.map((item, index) => (
              <div 
                key={index} 
                className={`${getPriorityColor(item.priority)} border-2 rounded-lg p-6 hover:shadow-md transition-shadow relative`}
              >
                {/* High Priority Warning */}
                {item.priority === 'high' && (
                  <div className="absolute top-2 right-2">
                    <div className="bg-red-600 text-white px-3 py-1 rounded-full text-xs font-bold flex items-center animate-pulse">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center mb-2">
                      ...
                      <span className={`ml-3 px-2 py-1 rounded text-xs font-bold ${
                        ...
                        {item.priority?.toUpperCase() || 'LOW'} PRIORITY
                      </span>
                    </div>
